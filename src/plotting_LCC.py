...
for j, file in enumerate(phasespace_files):
    ...
    ax1.annotate(r'$t_{\rm sim}\sim %.02f$ Myr'%(times[j]), xy=(0.7, 0.85), xycoords='axes fraction', fontsize=12)
    ...
    fig.tight_layout()
    plt.savefig('LCC_scatter_%s.pdf'%(str(j).rjust(6, '0')))
    plt.close()
    ...
    ax2.plot(rvals_plot_naught, hist_naught, label=r'$n(r, t\sim 15.0 \, {\rm Myr})$', linewidth=0.5)
    
    if j > 0:
        
        ax2.plot(rvals_plot, hist, label=r'$n(r, t\sim %.02f \, {\rm Myr})$'%(times[j]), linewidth=0.5)
    
    ax2.axvline(x=hl_radius, label=r'$r_{\rm half-light}(t\sim 15.0 \, {\rm Myr})$', c='r', linewidth=0.5)
    ax2.axvline(x=Jacobi_radius, label=r'$r_{\rm tidal}(t\sim %.02f \, {\rm Myr})$'%(times[j]), c='g', linewidth=0.5)
    ...
    fig.tight_layout()
    fig.savefig('LCC_number_density_%s.pdf'%(str(j).rjust(6, '0')))
    plt.close()