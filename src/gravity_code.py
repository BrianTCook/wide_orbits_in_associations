...
from EFF_with_clumps import enclosed_mass
from nemesis import *
...
def solver_codes_initial_setup(code_name, galaxy_code):
    ...
    filename = '/home/brian/Desktop/wide_orbits_in_associations/data/LCC_stars_ICs.csv'
    stars = read_set_from_file(filename, "csv")
    
    x_med, y_med, z_med = np.median(stars.x), np.median(stars.y), np.median(stars.z)
    ...
    converter_parent = nbody_system.nbody_to_si(Mgal, R_GC)
