...
def simulation(nGas, nStars, diskMass, rMin, rMax, Q, diskmassfrac, tEnd, dt):
	...
	for i, t in enumerate(sim_times):
		...
		plt.scatter(xvals_gas, yvals_gas, s=1, marker=',', c=colors_gauss, cmap=cm, linewidths=0, label='Protoplanetary Disk')
		plt.scatter(xvals_stars_and_planets, yvals_stars_and_planets, s=8, marker='.', c='r', label='Host star, planets')
		plt.xlim(-150., 150.)
		plt.ylim(-150., 150.)
		plt.xlabel(r'$x$ (AU)', fontsize=12)
		plt.ylabel(r'$y$ (AU)', fontsize=12)
		plt.annotate(r'$t_{\mathrm{sim}} = %.02f$ yr'%(t.value_in(units.yr)), xy=(0.05, 0.95), xycoords='axes fraction', fontsize=6)
